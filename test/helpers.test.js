...
import {pointsToBuffer, loadImage, setupWebGL, createProgram} from '../src/helpers.js';
...
describe('helpers', () => {
   ...
   */
  describe('one dimensional pointsToBuffer', () => {
    it('buffer type not eq with Type', () => {
      const points = [1, 2, 3];
      function fn() {
    ...
    it('default Type and null buffer', () => {
      const points = [1, 2, 3];
      const result = pointsToBuffer(points);
      assert.instanceOf(result, Float32Array);
      assert.deepEqual(result, new Float32Array(points));
    });

    it('default Type and 0 length buffer', () => {
      const points = [1, 2, 3];
    ...
    it('default Type and enought length buffer', () => {
      ...
      const result = pointsToBuffer(points, Float32Array, buffer);
      assert.deepEqual(result, buffer);
    });

    it('points type eq Type', () => {
  ...
  describe('two demensional pointsToBuffer', () => {
    it('default Type and null buffer', () => {
      const points = [
        [1, 2],
        [3, 4],
  ...
  describe('fetch API', () => {
    ...
    it('image3 useImageBitmap', async () => {
  ...
  describe('createProgram', () => {
    it('gl context with vertex and fragment shader', () => {
      const canvas = document.createElement('canvas');
    ...
    it('not gl context', () => {
      ...
      assert.throws(fn, TypeError, "Cannot read property 'createShader' of undefined");
    });
